...
@section('title',trans('main.show') . trans('main.teachers') . trans('main.section'))
@section('content')
    <div class="container">
        <!--begin::Card-->
        <div class="card card-custom">
            <div class="card-header flex-wrap border-0 pt-6 pb-0">
            <h1 style="font-size : 3rem">الاساتذة</h1>
            </div>
            <div class="card-body">
                <div class="mb-7">
    ...
    <script>
         ...
         var datatable = $('#kt_datatable').KTDatatable({
            ...
            data: {
                ...
                source: {
                    read: {
                        ...
                        url: "{!! route('getTeacherSectionDatatable.data') !!}",
                        data: {'id' : id},
                        method: 'POST',
                        headers: {
            ...
            columns: [{
                ...
                field: 'full_name', title: 'الاسم الكامل',
            },
            {
                field: 'subject', title: 'المادة',
            },

             {
                ...
                template: function(data) {
                    return '\
                    <a href="{{ url('section')}}/'+data.id+'/teachers" class="btn btn-sm btn-clean btn-icon mr-2" title="{{trans('main.show')}}{{trans('main.students')}}">\
...
<script>
    ...
    var KTDatatableMarkModal = function() {

        var initDatatable = function() {
            var el = $('#kt_datatable_marks');
            ...
            var datatable = el.KTDatatable({
                ...
                search: {
                    input: el.find('#kt_datatable_marks_search_query'),
                    key: 'generalSearch'
                },
            ...
            $('#kt_datatable_marks_search_status').on('change', function() {
                datatable.search($(this).val().toLowerCase(), 'Status');
            });

            $('#kt_datatable_marks_search_type').on('change', function() {
                datatable.search($(this).val().toLowerCase(), 'Type');
            });

            $('#kt_datatable_marks_search_status, #kt_datatable_marks_search_type').selectpicker();

            datatable.on('click', '[data-record-id]', function() {
                initSubDatatable($(this).data('record-id'));
                $('#MarksDatatable').modal('show');
            });
        };
        ...
        var initDatatableModal3 = function() {
            var modal = $('#kt_datatable_marks_modal');
            ...
            var datatable = $('#kt_datatable_marks').KTDatatable({
                ...
                search: {
                    input: modal.find('#kt_datatable_marks_search_query'),
                    delay: 400,
                    key: 'generalSearch'
                },
                ...
                columns: [{
                    ...
                    template: function(data, i) {
                        ...
                        var user_img = 'background-image:url(\'assets/media/users/100_' +
                            number + '.jpg\')';
                    ...
                    template: function(row) {
                        ...
                        return '<span class="label ' + status[row.Status].class +
                            ' label-inline label-pill">' + status[row.Status].title +
                            '</span>';
                    },
                }, {
                    field: 'Type',
                    ...
                    template: function(row) {
                        ...
                        return '<span class="label label-' + status[row.Type].state +
                            ' label-dot"></span>&nbsp;<span class="font-weight-bold text-' +
                            status[row.Type].state + '">' +
                            status[row.Type].title + '</span>';
                    },
                }, {
            ...
            $('#kt_datatable_marks_search_status_4').on('change', function() {
                datatable.search($(this).val().toLowerCase(), 'Status');
            });

            $('#kt_datatable_marks_search_type_4').on('change', function() {
                datatable.search($(this).val().toLowerCase(), 'Type');
            });

            $('#kt_datatable_marks_search_status_4, #kt_datatable_marks_search_type_4').selectpicker();
